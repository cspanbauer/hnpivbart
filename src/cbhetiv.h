...
)
{
   ...
   // bmf1 ouput storage
   Rcpp::NumericMatrix df1(nd,n); //f draws on train
   ...
   bmf2.setprior(alpha,mybeta,tauf22,tauf21);
   ...
   Rcpp::NumericMatrix df22(nd,n);
   Rcpp::NumericMatrix df21(nd,n);
         ...
         if(j % ke == 0){
           ...
           for(size_t k=0;k<n;k++) {
             if(include_output==1){               
               ...
               dsY(draw,k) = tmat[k][4];
               dLL(draw,k) = bvnorm(T[k],Y[k],bmf1.f(2*k),df22(draw,k)+df21(draw,k)*T[k],dsigma1(draw,k),dsigma2(draw,k),dcov(draw,k));
             }
           }
