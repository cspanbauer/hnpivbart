...
)
{
   ...
   Rcpp::NumericVector betasv(_betas);
   double *betas = &betasv[0];
   ...
   bmf.setdata(pzx,n*2,zx2,ytempf,zeroVp,nc);
   ...
   for(size_t i=0; i<n; i++) {
      ytempf[2*i] = T[i] - mTs;
      svecf[2*i] = sTs;
      R = (sTs/gammas)*(Y[i] - mYs - hs[i] - betas[i]) - T[i] + mTs;
      ytempf[2*i+1] = -R;
   ...
   bmh.setdata(px,n,x,ytemp,zeroVp,nc);
   ...
   for(size_t i=0;i<burnh;i++) {
      ...
      for(size_t j=0;j<n;j++) {
         ...
         ytemp[j] = Y[j] - mYs - betas[j] - gammas * ZZ1;
         svec[j] = sYs; 
      }
      //draw -----
      bmh.draw(svec,gen);
   ...
   for(size_t i=0;i<n;i++) {
      ...
      ytemp[i] = Y[i] - mYs - hs[i] - betas[i] - gammas * ZZ1;
      svec[i] = sYs; 
   }
   ...
   gp gaussProcess;
   gaussProcess.setdataX(n,T,betas);
   gaussProcess.setdataY(Y);
   double *gpSvec = new double[n];
   double *gpY = new double[n];
   double *gpX = new double[n];
   //--------------------------------------------------
   //--------------------------------------------------
   // beta setup
   ...
   /* } */
   /* double betad = 0.0; */
   Rcpp::NumericMatrix dGP(nd,n); //storage for output beta draws */
   ...
   for(size_t i=0;i<(nd+burn);i++) {
     //     cout << "Iter: " << i << std::endl;
      ...
      for(size_t j=0;j<n;j++) {
         ...
         ytemp[j] = Y[j] - mY - gaussProcess.f(j) - gamma * ZZ1;
         svec[j] = sY; 
      }
      } else {
      for(size_t j=0;j<n;j++) {
      ...
      for(size_t j=0; j<n; j++) {
         ...
         cout << "Y[j]: " << Y[j] << std::endl;
         cout << "mY: " << mY << std::endl;
         cout << "gp[j]: " << gaussProcess.f(j) << std::endl;
         cout << "T[j]: " << T[j] << std::endl;
         cout << "mT: " << mT << std::endl;
         R = (sT/gamma)*(Y[j] - mY - gaussProcess.f(j) - 0.0) - T[j] + mT;
         ytempf[2*j+1] = -R;
      ...
      for(size_t j=0; j<n; j++) {
         ytempf[2*j] = T[j] - mTs;
         svecf[2*j] = sTs;
         R = (sTs/gammas)*(Y[j]-mYs-betas[j] - hs[j]) - T[j] + mTs;
         ytempf[2*j+1] = -R;
      ...
      for(size_t j=0;j<n;j++){
        mT = tmat[j][0]; mY = tmat[j][1]; sT = tmat[j][2]; gamma = tmat[j][3]; sY = tmat[j][4];
        gpY[j] = Y[j]-mY-0.0-(gamma/sT)*(T[j]-mT-bmf.f(2*j));
        //        cout << j << ": " << gpY[j] << std::endl;
        gpSvec[j] = sY;
      }
      gaussProcess.setdataY(gpY);
      gaussProcess.draw(gpSvec,gen);
      ...
      for(size_t j=0;j<n;j++) {
         ...
         yS[2*j+1] = Y[j] - gaussProcess.f(j) - 0.0;
      }
      //draw -----
      dpmS.draw(gen);
