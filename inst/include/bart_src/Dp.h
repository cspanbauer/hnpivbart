...
public:
   ...
   //   y is pxn
  Dp():theta(),n(0),p(0),y(nullptr),alpha(1.0),q(0),eta(nullptr),etaconst(),ag(),priag(),lag(),lgr(),doDP(true) {}
  Dp(size_t _n, dv& _theta, bool _doDP);
   virtual ~Dp() {}
...
Dp::Dp(size_t _n, std::vector<double>& _theta, bool _doDP) : theta(),n(0),p(0),y(nullptr),alpha(1.0),q(0),eta(nullptr),etaconst(),ag(),priag(),lag(),lgr(),doDP(_doDP)
{
   n = _n;
...
public:
   ...
   //--------------------------------------------------
   //constructors/dest
  thetaGP():theta(),logLikelihood(),ind() {}
  thetaGP(const std::vector<double>& _theta):theta(_theta),ind() {}
  thetaGP(const std::vector<double>& _theta, const std::list<size_t>& _ind):theta(_theta),ind(_ind) {}
   ~thetaGP() {}
   ...
   //data members
   std::vector<double> theta; //the "theta" value
  std::vector<double> logLikelihood; // log-likelihood value same length as 'ind', one for each observation in cluster
   std::list<size_t> ind;  //which observations have this theta value
};
...
void printdvv(Dp::dvv& m);
void printdv(Dp::dv& v);
double logSumExp(std::vector<double>& lg, double mx);
size_t drind(double sum, Dp::dv& pv, rn& gen);
double logam(double x);
...
Dp::Dp(size_t _n, std::vector<double>& _theta, bool _doDP) : theta(),n(0),p(0),y(nullptr),alpha(1.0),q(0),eta(nullptr),etaconst(),ag(),priag(),lag(),lgr(),doDP(_doDP)
{
   ...
   for(size_t i=0;i<n;i++) {
      th.ind.push_back(i);
      th.logLikelihood.push_back(0.0);
   }
...
//###--------------------------------------------------
//### get vector of log-likelihoodt values, one for each observation in a vector of size n
Dp::dv Dp::logLikeVector() 
{
   auto ii = theta.begin();
   size_t p = ii->theta.size();
   //cout << "in thetaVector, p is: " << p << std::endl;
   ...
   Dp::dv v(n);
   for(titer ip=theta.begin(); ip!=theta.end();ip++) { // loop over partitions
     std::vector<double>::iterator ll = ip->logLikelihood.begin();
     for(thetaGP::iiter j=ip->ind.begin();j!=ip->ind.end();j++) {
       size_t obsn = *j;
       v[obsn] = *ll;
       ll++;
     }
...
void Dp::drawTheta(rn& gen)
{
   ...
   */
   ...
   for(size_t i=0;i<np;i++) {
      thetaGP::iiter ob = par->ind.begin();
      std::vector<double>::iterator ll = par->logLikelihood.begin();
      for(size_t j=0;j<icnt[i];j++) { 
         size_t nts = tst.size();
         std::vector<double> pvl(nts + 1);
	 std::vector<double> pv(nts+1);
	 double mx=pvl[0];
	 double sum = 0.0;
         for(size_t k=0;k<nts;k++) {
            if(cnt[k] == 0) { //tricky, because you drop empties at the end
	      pvl[k] = -std::numeric_limits<double>::infinity(); //exp(pvl[k])=0 if pvl[k]=-inf
	      pv[k] = 0.0;
	    } else {
               size_t thecnt = cnt[k];
               if(tst[k]==par->theta) thecnt -=1; //par->theta will be the theta of the obs you are on
               if(eta == nullptr) {
		 pvl[k] = log(thecnt)+f(y+(*ob)*p,tst[k],&etaconst[0]);
               } else {
		 pvl[k] = log(thecnt)+f(y+(*ob)*p,tst[k],eta+(*ob)*q);
               }
	       if(pvl[k]>mx) mx=pvl[k];
	       pv[k] = exp(pvl[k]);
	       sum += pv[k];
            }
         }
         if(eta == nullptr) {
	   pvl[nts] = lalpha+qo(y+(*ob)*p,&etaconst[0]);
         } else {
	   pvl[nts] = lalpha+qo(y+(*ob)*p,eta+(*ob)*q);
         }
	 if(doDP){
	   if(pvl[nts]>mx) mx=pvl[nts];
	   pv[nts]=exp(pvl[nts]);
	   sum += pv[nts];
	 }
	 else{
   ...
   */
         if(ii==nts) {  // birth
            ...
            //add new cluster (of size 1) to theta
            thetaGP ngp; ngp.theta = ntheta; ngp.ind.push_back(*ob); ngp.logLikelihood.push_back(loglike);
            theta.push_back(ngp);

            //manage ob
            ob = par->ind.erase(ob);
         } else if(tst[ii] == par->theta) { //drew yourself, update log-likelihood and go on to next observation
	   ob++;
         } else {  //drew one of the other old thetas
            //update cnt
            cnt[i] -=1; cnt[ii] += 1;
