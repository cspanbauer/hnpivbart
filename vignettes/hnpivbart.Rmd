...
For the instrumental variable case, that means specifying the instrument vector/matrix <tt>Z</tt>, the exposure vector <tt>T</tt>, the outcome vector <tt>Y</tt>, and the vector/matrix of other covariates <tt>X</tt>. The outputs will depend on the model specified. In all cases, the first stage is returned via element <tt>df1</tt> from the resulting BART model fit. The outputs, called <tt>df21</tt>, <tt>df22</tt>, and <tt>df2</tt>, otherwise vary depending on the functional form chosen for stage 2. These outputs are given in Table \@ref(tab:foo). Note that <tt>hetnpivbart</tt> does not output <tt>df21</tt> or <tt>df22</tt> as these functions are not estimated separately.

Table: \@ref(tab:foo) Your table caption.
    
|                      | <tt>df1</tt>     | <tt>df21</tt>       | <tt>df22</tt>       | <tt>df2</tt>                             |
|----------------------|------------------|---------------------|---------------------|------------------------------------------|
| <tt>ivbart</tt>      | $f_1(\mathbf{z}_i)$ | $\beta t_i$         | $f_{22}(\mathbf{x}_i)$ | $\beta t_i + f_{22}(\mathbf{x}_i)$          |
| <tt>hetivbart</tt>   | $f_1(\mathbf{z}_i)$ | $f_{21}(\mathbf{x}_i)$ | $f_{22}(\mathbf{x}_i)$ | $f_{21}(\mathbf{x}_i)t_i+f_{22}(\mathbf{x}_i)$ |
| <tt>npivbart</tt>    | $f_1(\mathbf{z}_i)$ | $f_{21}(t_i)$       | $f_{22}(\mathbf{x}_i)$ | $f_{21}(t_i)+f_{22}(\mathbf{x}_i)$          |
| <tt>hetnpivbart</tt> | $f_1(\mathbf{z}_i)$ |                     |                     | $f_{2}(t_i,\mathbf{x}_i)$                   |
...
First, the data is simulated according to the specifications in Spanbauer and Pan (20XX). The only difference is that normal random variates are used as the instruments instead of SNPs from gene FTO as in the article. This is done primarily for simplicity, but will not affect the results. We are using the global linear setting here so that $f_2(t_i,\mathbf{x}_i)=t_i+I(x_{i1}<0)$.
...
Now the line is replaced with the cosine function so that $f_2(t_i,\mathbf{x}_i)=\cos(t_i)+I(x_{i1}<0)$.
...
The original line is now interacting with $\mathbf{x}_{1i}$ so that $f_2(t_i,\mathbf{x}_i)=t_iI(x_{i1}<0)$.
...
The nonlinear cosine function is now multiplicative with $\mathbf{x}_{1i}$ so that $f_2(t_i,\mathbf{x}_i)=t_iI(x_{i1}<0)$.        
